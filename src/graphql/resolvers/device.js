...
module.exports = {
	...
	registerDevice: async (args, req) => {
		...
		const existDevice = await Device.findOne({ secretKey: args.deviceInput.secretKey });
		if (existDevice) {
	...
	updateSensor: async (args, req) => {
		...
		try {
			const device = await Device.findOne({ secretKey: args.sensorInput.secretKey });
			if (!device) {
	...
	removeDevice: async (args, req) => {
		...
		// }
		try {
			const device = await Device.findByIdAndDelete(args.deviceId).populate('owner');
